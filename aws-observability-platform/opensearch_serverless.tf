...
# ⚠️ 런북 기능 미사용으로 전체 주석 처리 (AOSS OCU 비용 절감)
# 활성화 시 아래 주석 해제 후 terraform apply
# ============================================================
...
#   policy = jsonencode({
#     Rules = [{ ResourceType = "collection", Resource = ["collection/${var.project_name}-runbooks"] }]
#     AWSOwnedKey = true
#   })
# }
#
...
#   policy = jsonencode([{
#     Rules = [
#       { ResourceType = "collection", Resource = ["collection/${var.project_name}-runbooks"] },
#       { ResourceType = "dashboard",  Resource = ["collection/${var.project_name}-runbooks"] }
#     ]
...
#   policy = jsonencode([{
#     Rules = [
#       { ResourceType = "index",      Resource = ["index/${var.project_name}-runbooks/*"], Permission = ["aoss:*"] },
#       { ResourceType = "collection", Resource = ["collection/${var.project_name}-runbooks"], Permission = ["aoss:*"] }
#     ]
#     Principal = [
...
#     Version = "2012-10-17"
#     Statement = [
#       { Sid = "S3RunbooksAccess", Effect = "Allow", Action = ["s3:GetObject", "s3:ListBucket"],
#         Resource = [aws_s3_bucket.runbooks.arn, "${aws_s3_bucket.runbooks.arn}/*"] },
#       { Sid = "BedrockEmbedding", Effect = "Allow", Action = ["bedrock:InvokeModel"],
...
#       fixed_size_chunking_configuration { max_tokens = 300, overlap_percentage = 20 }
#     }
#   }
# }
#
...
#     Version = "2012-10-17"
#     Statement = [
#       { Sid = "BedrockKBSync", Effect = "Allow", Action = ["bedrock:StartIngestionJob", "bedrock:GetIngestionJob"],
...
#       { Sid = "BedrockEmbedding", Effect = "Allow", Action = ["bedrock:InvokeModel"],
#         Resource = "arn:aws:bedrock:${var.aws_region}::foundation-model/amazon.titan-embed-text-v2:0" },
#       { Sid = "S3Runbooks", Effect = "Allow", Action = ["s3:GetObject", "s3:ListBucket"],
#         Resource = [aws_s3_bucket.runbooks.arn, "${aws_s3_bucket.runbooks.arn}/*"] }
#     ]
#   })
# }
