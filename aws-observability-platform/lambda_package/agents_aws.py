...
def get_service_name_from_source(src: dict) -> str:
    """OpenSearch 문서의 _source에서 서비스명 추출.
    direct opensearch exporter: resource 오브젝트 안에 service.name으로 저장됨
    """
    return (
        src.get("resource", {}).get("service.name")
        or "unknown"
    )
...
def query_amp(promql: str, time_range_minutes: int = 60) -> list:
    ...
    try:
        ...
        url = (
            f"{base}/query_range"
            f"?query={urllib.parse.quote(promql, safe='')}"
...
def query_amp_instant(promql: str) -> list:
    ...
    try:
        ...
        url = f"{base}/query?query={urllib.parse.quote(promql, safe='')}"
        request = AWSRequest(method="GET", url=url)
        SigV4Auth(credentials, "aps", AWS_REGION).add_auth(request)
    ...
    query = {
        ...
        "aggs": {
            "by_service": {
                "terms": {
                    "script": {
                        "source": "def r = params._source.get('resource'); if (r != null && r.containsKey('service.name')) return r.get('service.name'); return 'unknown'",
                        "lang": "painless"
                    },
                    "size": 10
                },
                "aggs": {
                    "by_severity": {
                        "terms": {"field": "severity.text.keyword", "size": 5}
                    }
                }
            }
        }
...
def search_logs(severity: str = "", keyword: str = "") -> str:
    ...
    Args:
        ...
        keyword: 검색 키워드
    """
    must_clauses = [{"range": {"@timestamp": {"gte": "now-1h"}}}]

    if severity:
        must_clauses.append({"term": {"severity.text.keyword": severity}})
    if keyword:
    ...
    for h in hits:
        ...
        svc = get_service_name_from_source(src)
        sev = src.get("severity", {}).get("text", "")
        body = src.get("body", "")
        ...
        "query": {
            "bool": {
                "must": [
                    ...
                    {"terms": {"severity.text.keyword": ["ERROR", "WARN"]}}
                ]
            }
        },
        "sort": [{"@timestamp": {"order": "desc"}}]
...
def get_traces_summary() -> str:
    ...
    result = query_opensearch("traces-app", query)
    ...
    for b in buckets:
        ...
        lines.append(f"[{service}]")
        lines.append(f"  span 수: {count}개, 에러율: {error_rate:.1f}%")
...
def get_slow_spans(threshold_ms: float = 100.0) -> str:
    ...
    # durationInNanos 필드 없음 → endTime - startTime으로 계산
    slow_spans = []
    for h in hits:
        src = h.get("_source", {})
        try:
            start = datetime.fromisoformat(src.get("startTime", "").replace("Z", "+00:00"))
            end   = datetime.fromisoformat(src.get("endTime",   "").replace("Z", "+00:00"))
            duration_ms = (end - start).total_seconds() * 1000
        except Exception:
            duration_ms = 0.0
        if duration_ms >= threshold_ms:
            slow_spans.append((src, duration_ms))

    slow_spans.sort(key=lambda x: x[1], reverse=True)

    if not slow_spans:
        return f"threshold {threshold_ms}ms를 초과하는 slow span이 없습니다."

    lines = [f"=== Slow Spans (>{threshold_ms}ms) ==="]
    for src, duration_ms in slow_spans[:20]:
        service = get_service_name_from_source(src)
